# PocketTmux — AGENTS.md

Build/install notes for this repo. General contributing workflow lives in [CONTRIBUTING.md](CONTRIBUTING.md); design docs in `docs/` (start with `docs/PRODUCT.md`, `docs/ARCHITECTURE.md`, and `docs/UI_GUIDELINES.md`; the wire format is `docs/PROTOCOL.md`).
...
App/PocketTmuxKit/     Swift package: PocketTmuxKit (protocol v2, tmux window/pane parsers, pairing) + PocketTmuxAgent (server, tmux -CC)
...
- **Xcode 26.2** and **tmux 3.7** are the baselines. SwiftTerm is resolved at **1.20.0** (`Project.yml`: `from: 1.19.0`). The pane client uses tmux 3.7's `active-pane` and `switch-client -Z` semantics.
...
## Native UI contract — read before touching app views

- Product UI uses **Apple-standard SwiftUI components and platform conventions**, not a custom visual theme. The full rationale, component map, review checklist, and official sources are in `docs/UI_GUIDELINES.md`.
- Do not add a fixed app palette, `.preferredColorScheme(.dark)`, fixed ordinary-text sizes, custom section headers, custom navigation bars, or hand-built replacements for `List`, `Form`, `NavigationLink`, sheets, toolbars, menus, alerts, and standard empty states.
- Respect Light/Dark Mode, Increase Contrast, Dynamic Type, system accent color, keyboard/focus behavior, and accessibility labels.
- Monospaced styling and custom visuals are limited to terminal/log content, QR scanning/rendering, and technical values.
- Protocol/tmux logic still belongs in `PocketTmuxKit`; UI-only presentation may live in the relevant app target.
...
The e2e script creates and kills its own tmux sessions (`pt_alt`, `pt_norm`, `pt_mouse`, `pt_win`, `pt_pane`). It strips inherited `TMUX` / `TMUX_PANE` before every tmux subprocess, so it is safe to run from a tmux shell.
...
## Panes — read before touching split behavior

- A window is not a tab on the phone. `TmuxMenu` exposes **Window** and **Pane**
  as separate native submenus; the agent sends `panes{sessionID,windowID,panes}`
  and accepts `pane.select{id}`.
- The control client is spawned with `attach-session -f active-pane,ignore-size`.
  Input is sent with an explicit pane id, so the phone keeps its selected pane.
- A split window has one geometry. Pinning a 45-column phone window while two
  panes are side-by-side leaves each pane ~22 columns. `TmuxControl` therefore
  uses `resize-pane -Z` / `switch-client -Z` to give the selected pane the full
  phone grid, records only zoom state it introduced, and unzooms on window switch
  or detach. **Never drop the zoom step for multi-pane windows** — that is the
  half-clipped-TUI regression in `docs/TROUBLESHOOTING.md` §4.
- `layout-change` is not a command to repaint unconditionally: the agent's own
  `refresh-client -C` can produce layout notifications. Refresh only when the
  selected pane disappeared, the split is unzoomed, or its size differs from the
  phone; otherwise repainting creates a control-event loop.
- Short-lived commands (`TmuxRunner`) and the forked control client must remove
  `TMUX` and `TMUX_PANE`. If the agent is launched from a tmux shell, inherited
  values make commands run in the launching client's context — see
