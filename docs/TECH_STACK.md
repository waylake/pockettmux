...
*Every component, with version · license · why it was chosen · the alternative that was considered. Versions verified **2026-09-01/02** via GitHub API and the development machine's toolchain; tmux pane-control behavior re-verified **2026-09-24**.*
...
3. **Native on both ends.** SwiftUI on iPhone and Mac, using Apple-standard components and semantic styling around the terminal; no webview as a terminal surface. See [UI_GUIDELINES.md](UI_GUIDELINES.md).
...
| UI | SwiftUI standard components (+ UIKit for the terminal view and camera) | iOS 16+ | — | Native navigation, lists, forms, sheets, toolbars, empty states, Dynamic Type, and appearance adaptation; UIKit only where the platform view is specialized |
...
| tmux bridge | **Control mode (`tmux -CC`)** on a `forkpty` pty + `capture-pane` priming | — | — | Event-driven `%output`, stable ids, client-scoped `active-pane`, `switch-client -Z`, reversible `resize-pane -Z`, and `refresh-client -C`. *See D4/D11.* |
...
| Linter | **SwiftLint** + native UI contract | 0.65.1 | `.swiftlint.yml` plus `scripts/check-native-ui.sh`; CI runs both, and the UI contract/research lives in `docs/UI_GUIDELINES.md` |
...
| E2E | `scripts/check-attach-prime.py` (raw WebSocket client, stdlib Python) | — | Attach/prime/windows/split-pane zoom-selection-restoration/input/paste/auth against a live agent and real tmux |
...
- **tmux 3.7c** — released 2026-08-17. Control-mode, `active-pane`, client switching, and zoom facts in PROTOCOL §5 / TROUBLESHOOTING were verified against it.
...
| SwiftTerm build-tool plugin fails validation on Xcode 26.2 | CLI builds abort before compiling | `-skipPackagePluginValidation` everywhere (scripts, CI, AGENTS.md) |
...
The "why" behind each row is the decision in [ARCHITECTURE.md → §6](ARCHITECTURE.md#6-key-technical-decisions--alternatives) (D1–D11) and the failure notes (§7, F1–F14).
